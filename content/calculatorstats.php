...
<p>The method in which we measure the amount of calories burnt is done using "METS"</p>
...
$find_sql = "SELECT * FROM `Menu`  JOIN Item ON (`Item`.`Item_ID` = `Menu`.`Item_ID` ) WHERE `ID` = '$ID'";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $ID_met = mysqli_real_escape_string($dbconnect, $_POST['METS']);
    ...
    if ($has_errors != "yes") {
        ...
        $met_sql = "SELECT * FROM `MET` JOIN Activity ON (`Activity`.`Activity_ID` = `MET`.`Activity_ID`)  WHERE `ID` = '$ID_met' ";
        ...
        <?php
        if($hours_needed == "0"){
            echo $calories . " cal of calories in a "?><b><?php echo " $food"?></b><?php ; ?> <br /> <br />
...
?>
<form autocomplete="off" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"] . "?page=calculatorstats&ID=$ID"); ?>" enctype="multipart/form-data">
    <b>Activity:</b>
    <br />
    ...
    <input class="add-field <?php echo $calories_field ?>" type='text' name='calories' placeholder="Calories(cal)" value="<?php echo $find_rs['Calories']; ?>" />
    <br />
